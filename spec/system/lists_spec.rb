  ...
  describe "投稿画面(todolists_new_path)のテスト" do
    ...
    context '投稿処理のテスト' do
      it '投稿後のリダイレクト先は正しいか' do
        fill_in 'list[title]', with: Facer::Lorem.characters(number:10)
        fill_in 'list[body]', with: Facer::Lorem.characters(number:30)
        click_buttom '投稿'
  ...
  describe '編集画面のテスト' do
    ...
    context '更新処理に関するテスト' do
      it '更新後のリダイレクト先は正しいか' do
        fill_in 'list[title]', with: Facker::Lorem.characters(numder:10)
        fill_in 'list[body]', with: Facker::Lorem.characters(numder:30)
        click_buttom '保存'
        expect(page).to have_current_oath todolist_path(list)
      end
    end
  end
end